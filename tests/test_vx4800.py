...
def test_generated_presentation_regression():
    data=build(); study=json.loads((ROOT/'fixtures/vx4800/presentation/v5.2.0/study.json').read_text())
...
def test_viewer_build():
    ...
    p=ROOT/'build/vx4800/VX4800_VORTEX_Viewer_v5.2.0.html'; s=p.read_text()
...
def test_release_is_reproducible():
    subprocess.run([sys.executable,str(ROOT/'scripts/build_release.py')],check=True)
    p=ROOT/'build/vx4800/AETHERIA_VORTEX_v5.2.0.zip'; h1=hashlib.sha256(p.read_bytes()).hexdigest()
    subprocess.run([sys.executable,str(ROOT/'scripts/build_release.py')],check=True)
    h2=hashlib.sha256(p.read_bytes()).hexdigest(); assert h1==h2
    with zipfile.ZipFile(p) as z: assert z.testzip() is None and 'coordination/setout-coordination-v1.3.0.dxf' in z.namelist()

def test_photometry_is_not_misrepresented():
    p=json.loads((ROOT/'fixtures/vx4800/photometry/concept-v5.2.0.json').read_text())
    assert p['status']=='conceptual'; assert p['futureMeasuredData']['iesRequired'] is True
...
def test_release_checksums_are_standard_newline_text(built_release):
    with zipfile.ZipFile(built_release) as z:
        checks=z.read('SHA256SUMS.txt').decode('utf-8')
        assert '\\n' not in checks
        lines=[line for line in checks.splitlines() if line]
        assert len(lines)>=10
        for line in lines:
