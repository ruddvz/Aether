...
for slug, product in P['products'].items():
    ...
    presentation_dl = OUT / 'downloads' / slug / version
    presentation_dl.mkdir(parents=True, exist_ok=True)
    shutil.copy2(release, presentation_dl / release.name)
    ...
    if optimized_glb.exists(): shutil.copy2(optimized_glb, design_dl / optimized_glb.name)
    if optimization_manifest.exists(): shutil.copy2(optimization_manifest, design_dl / optimization_manifest.name)
    ...
    meta = {
        ...
        'downloadPath': f'downloads/{slug}/{version}/{release.name}',
...
cards = ''.join(
    f'''<article class="card"><div class="ey">{html.escape(x['model'])}</div><h2>{html.escape(x['displayName'])}</h2><p>Design {x['designRevision']} · presentation {x['presentationRevision']}</p><div class="actions"><a href="{x['stablePath']}">Open viewer →</a>{f'<a href="{x["inspectorPath"]}">Technical inspector →</a>' if x['inspectorPath'] else ''}</div></article>'''
    for x in registry
)
