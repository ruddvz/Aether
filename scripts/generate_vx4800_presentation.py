...
V52_RELEASE_POSE_PATH=FIX/'presentation/v5.2.0/released-pose.json'
V52_POSE_FIELDS=('yaw','foldL','foldR','roll','pitch')
...
def _load_v52_release_pose():
    snapshot=json.loads(V52_RELEASE_POSE_PATH.read_text())
    if snapshot.get('presentationRevision')!='5.2.0':
        raise RuntimeError('V5.2 released pose snapshot has the wrong presentation revision')
    if tuple(snapshot.get('poseFields',[]))!=V52_POSE_FIELDS:
        raise RuntimeError('V5.2 released pose snapshot fields do not match the generator contract')
    return snapshot


def _freeze_v52_release_pose(elements, snapshot):
    by_id={e['id']:e for e in elements}
    released=snapshot.get('elements',{})
    if set(released)!=set(by_id):
        missing=sorted(set(by_id)-set(released))
        extra=sorted(set(released)-set(by_id))
        raise RuntimeError(f'V5.2 released pose coverage mismatch: missing={missing}, extra={extra}')
    for element_id, values in released.items():
        if set(values)!=set(V52_POSE_FIELDS):
            raise RuntimeError(f'V5.2 released pose fields are incomplete for {element_id}')
        for field in V52_POSE_FIELDS:
            by_id[element_id][field]=values[field]
...
def build():
    ...
    if study['presentationRevision']=='5.2.0':
        release_pose=_load_v52_release_pose()
        if release_pose.get('expectedViewerDataSha256')!=study.get('expectedViewerDataSha256'):
            raise RuntimeError('V5.2 released pose snapshot is bound to a different viewer fingerprint')
        _freeze_v52_release_pose(elements,release_pose)
    ...
    if release_pose is not None and canonical_sha(out)!=release_pose['expectedViewerDataSha256']:
        raise RuntimeError('V5.2 generated viewer data no longer matches the immutable released fingerprint')
    return out
